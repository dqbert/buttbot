...
exports.guild.get = async function(guild) {
    ...
    assert.ok(guild instanceof discord.Guild, "Guild " + guild + " is not a valid guild!");
...
exports.admin.get = async function(guild, admin_defaults) {
    ...
    try {
        ...
        if (err.code === 'ENOENT') {
            assert.throws(() => {
                JSON.parse(JSON.stringify(admin_defaults));
            }, "Admin_defaults " + admin_defaults + " is not valid JSON!");

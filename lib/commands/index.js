...
var guild_create_admin = async function(message) {
    ...
    //load our config
    var guild_cfg = await config.guild.get(channel);
    ...
    if (role == null || !role) {
        role = await guild.roles.find(search_role => search_role.name === "Buttbot admins");
        try {
...
var need_admin_role = async function(message) {

    //create admin role or get it if it exists
    var admin_role = await guild_create_admin(message);
    if (admin_role == null && message.guild == null) {
        return false
    }
    else if (admin_role == null) {
        throw new Error("Unexpected error: null admin_role with existing message guild!");
    }
    ...
    if (!message.member.roles.has(admin_role.id)) {
        messaging.send("Error: you must be a member of the \"" + admin_role.name + "\" role to use this command!", message.channel, message.author);
        return true;
    }
    else {
        return false;
...
exports.process = async function(message) {
    ...
    if (COMMANDS_ALIASES.indexOf(command_name.toLowerCase()) > -1) {
        ...
        if (config.guild.fromChannel(message.channel) instanceof discord.Guild) {
            admin_commands = await config.admin.get(config.guild.fromChannel(message.channel), admin_defaults);
        }
    ...
    //check if command exists
        ...
        if (commands.get(command_name).permissions != null && message.guild != null) {
            ...
            if (!member.permissions.has(commands.get(command_name).permissions, true)) {
                messaging.send("Error: I don't have the correct permissions for this command! (Permissions I need: " + commands.get(command_name).permissions.toString() + ")", message.channel, message.author);
                return;
            }
        }
        ...
        if (message.guild instanceof discord.Guild) {
            admin_commands = await config.admin.get(message.guild, admin_defaults);

            for (var key in admin_commands) {
                if (!admin_commands.hasOwnProperty(key)) continue;
                if (key == command_name) {
                    if (admin_commands[key] == true) {
                        found_command = await need_admin_role(message);
                    }
                    break;
                }
            }
    ...
    //otherwise, command doesnt exist
        ...
        messaging.send("Invalid command: " + command_name + "!", message.channel, message.author);
        return;
    }
}

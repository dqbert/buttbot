...
exports.send = async function(message, channel, user, reply_to) {
    ...
    try {
        while (message.length > 2000) {
            promise = await channel.send(message.substring(0, 1999));
            message = message.substring(1999);
            //probably a result of a command, log it
            if (user instanceof discord.User) {
                logging.use_log(promise, reply_to);
            }
        }
        ...
        if (user instanceof discord.User) {
            ...
            logging.use_log(err_msg, reply_to);
        }
        else {

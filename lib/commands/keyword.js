...
exports.process = async function(message) {
    var VALID_SUBPARMS = await sql.keyword.get.keyword_types();
    ...
    if (subcommand == "help") {
        ...
        if (help_command == "add") {
            messaging.send(addusage, message.channel, message.author, message);
            return;
        }
        if (help_command == "list") {
            messaging.send(listusage, message.channel, message.author, message);
            return;
        }
        if (help_command == "delete") {
            messaging.send(delusage, message.channel, message.author, message);
            return;
        }
        else {
            messaging.send(usage, message.channel, message.author, message);
    ...
    //TODO: convert this to SQL
        ...
        if (keyword == "" || keyword == null) {
            messaging.send("Error: no keyword found!" + os.EOL + addusage, message.channel, message.author, message);
            return;
        }
        ...
        if (after_keyword == "" || after_keyword == null) {
            messaging.send("Error: no type specified after keyword!" + os.EOL + addusage, message.channel, message.author, message);
            return;
        }
        ...
        if (VALID_SUBPARMS.indexOf(subparm) == -1) {
            messaging.send("Error: invalid type specified after keyword: " + subparm + "!" + os.EOL + addusage, message.channel, message.author, message);
            return;
        }
        ...
        if (subparm == "notify") {
            after_text = "<@" + message.author.id + ">";
        }

        try {

            var result = await sql.keyword.add(
                config.guild.fromChannel(message.channel).id,
                message.author.id,
                keyword,
                subparm,
                after_text,
                null //to be replaced by command_id when that is supported
            );

            messaging.send(`Keyword added successfully!${os.EOL}` +
                           "```" + keyword + " [" + subparm + "]: " + after_text + "```",
            ...
            if (err.code === "EDUPKEYW") {
                messaging.send("Keyword not added: keyword defintion already exists!", message.channel, message.author, message);
            }
            else {
                messaging.send("Error adding keyword! Keyword not added to set of keywords.", message.channel, message.author, message);
        ...
        try {
            ...
            if (duplicate) {
                ...
                messaging.send("Keyword added successfully!", message.channel, message.author, message);
                messaging.send("```" + keyword_obj.keyword + " [" + keyword_obj.subparm + "]: " + keyword_obj.after_text + "```", message.channel, message.author, message);
            }
    ...
    //handle listing keywords
        ...
        var response = "";
        keyword = message.content.split(" ").splice(2).join(" ");
        ...
        var keywords = await sql.keyword.get.by_guild(message.guild.id, keyword);
        ...
        response = "Here are a list of keywords matching your search:" + os.EOL;

        keywords.forEach((keyword) => {
            response += `${keyword.keyword} [${keyword.keyword_type}]: ${keyword.keyword_text} ${os.EOL}`;
        });
        ...
        try {
            ...
            messaging.send("No keywords defined for this channel!", message.channel, message.author, message);
            setTimeout(async function() {
                throw err;
            });
        }
    ...
    //handle deleting keywords
        ...
        var removed_keywords = await sql.keyword.get.by_guild(message.guild.id, keyword, true);

        var result = await sql.keyword.remove(message.guild.id, keyword);

        if (!result.hasOwnProperty("affectedRows")) {
            messaging.send("!! Unexpected error occurred! Use b/help and join the help channel or message @dqbert#0903 !!", message.channel, message.author, message);
            throw new Error("affectedRows was not defined in result!");
        }

        if (result.affectedRows === 0) {
            messaging.send(`No keywords were removed matching search: ${keyword}`, message.channel, message.author, message);
            return;
        }

        var response = "Keywords deleted successfully!" + os.EOL + "```";

        removed_keywords.forEach((keyword) => {
            response += `${keyword.keyword} [${keyword.keyword_type}]: ${keyword.keyword_text} ${os.EOL}`;
        });
        ...
        try {
            ...
            if (deleted_response == "") messaging.send("No keywords match: " + keyword + " to delete!", message.channel, message.author, message);

            else {
                writeFile(path.resolve(GUILD_PATH, message.guild.id, 'keywords.json'), output, {flag: "w"});
                messaging.send("Keywords deleted successfully!", message.channel, message.author, message);
                messaging.send("```" + deleted_response + "```", message.channel, message.author, message);
            }
        }
        catch (err) {
            messaging.send("No keywords defined for this channel!", message.channel, message.author, message);

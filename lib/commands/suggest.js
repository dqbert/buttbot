...
exports.process = async function(message) {
    ...
    await console_commands.process("todo pending [g:" + config.guild.fromChannel(message.channel).id + " u:" + message.author.id + "] " + argv.slice(1).join(' '), message);
    messaging.send("Suggestion added successfully!", message.channel, message.author);
}

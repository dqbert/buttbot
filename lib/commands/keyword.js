...
exports.permissions = ["SEND_MESSAGES", "MANAGE_MESSAGES"];
...
exports.process = async function(message) {
    ...
    //handle adding a keyword
        ...
        try {
            ...
            if (duplicate) {
                ...
                messaging.send("```" + keyword_obj.keyword + " [" + keyword_obj.subparm + "]: " + keyword_obj.after_text + "```", message.channel);
            }
    ...
    //handle deleting keywords
        ...
        try {
            ...
            if (deleted_response == "") messaging.send("No keywords match: " + keyword + " to delete!", message.channel);

            else {
                writeFile(path.resolve(GUILD_PATH, message.guild.id, 'keywords.json'), output, {flag: "w"});
                messaging.send("Keywords deleted successfully!", message.channel);
                messaging.send("```" + deleted_response + "```", message.channel);
            }
        }
        catch (err) {
            messaging.send("No keywords defined for this channel!", message.channel);

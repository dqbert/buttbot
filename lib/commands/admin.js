...
exports.process = async function(message) {
    ...
    if (argv[1].toLowerCase() == "add" || argv[1].toLowerCase() == "remove" || argv[1].toLowerCase() == "toggle") {
        ...
        if (!admin_commands.hasOwnProperty(command_name)) {
            messaging.send("Error: command " + command_name + " is not valid!", message.channel, message.author, message);
            return;
        }
        ...
        messaging.send(confirmation, message.channel, message.author, message);
    }
    else {
        messaging.send(usage, message.channel, message.author, message);
        return;

...
exports.stripPrefix = async function(message)
{
    var prefix = await sql.guild.get_prefix(config.guild.fromChannel(message.channel).id);
    var content = message.content.replace(new RegExp(`^${prefix}`, 'g'), '');
    content = content.replace(new RegExp(`^<@${bot.user.id}> `, 'g'), '');
...
exports.process = async function(message) {
    ...
    reload(path.resolve(command.command_path)).process.apply(null, arguments);
    return;
}

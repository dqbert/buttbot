...
exports.guild.default = {
    ...
    "log_channel"   : "",   //channel which to send unsolicited messages
    "managed_roles" : []    //roles managed by buttbot role command
                            //each role stored as [key, value] array pair
};
...
exports.guild.get = async function(guild) {
    ...
    //TODO: uncomment this
    /*var guild_cfg = await sql.guild.get(guild.id);
    return guild_cfg;*/
    ...
    try {
        guild_file = await readFile(cfg_path);
        if (guild_file == null || guild_file.toString().trim() == "") {
            guild_file = await readFile(cfg_path);
        }
        guild_file = guild_file.toString();
        guild_cfg = JSON.parse(guild_file);
    }
    catch (err) {
        ...
        //something else went wrong
            ...
            setTimeout(async function() {
                logging.log(`error for config: ${cfg_path}:`);
                logging.log(`${guild_file}`);
                throw err;
            });
        }
    }
...
exports.guild.create_dir = async function(guild) {
    ...
    try {
        await mkdir(guild_dir);
        logging.log("Created new directory for guild: " + logging.guild_print(guild));
    }
    catch (err) {
        if (err.code != "EEXIST") {
            setTimeout(async function() {

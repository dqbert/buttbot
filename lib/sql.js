...
var reconnect = async function() {
    ...
    exports.query = util.promisify(exports.sql_connection.query).bind(exports.sql_connection);
...
exports.todo = {};
exports.todo.get = async function(todo_text, guild_id, user_id, top_amount) {
    ...
    quer = "SELECT t2.todo_index, t1.todo_text " +
           "FROM todo t1, todo_order t2 " +
           "WHERE t1.todo_id = t2.todo_id";

    if (conditions.length > 0) {

        for (var i = 0; i < conditions.length; i++) {
            quer = quer + " AND " + conditions[i];
        }
    }
    quer = quer + " ORDER BY t2.todo_index";
    quer = quer + limit;
...
exports.todo.swap = async function(index1, index2) {
    assert_nonnull(index1, "index1");
    assert_nonnull(index2, "index2");
    index1 = parseInt(index1);
    index2 = parseInt(index2);

    var id1 = await exports.query(
        "SELECT todo_id " +
        "FROM todo_order " +
        "WHERE todo_index = ?",
        [index1]
    ...
    if (id1 === null || id1 === undefined || id1.length === 0 ||
        id2 === null || id2 === undefined || id2.length === 0) {
        return null;
    }
...
exports.todo.add = async function(text, pending, user_id, guild_id)
{
    ...
    quer = "INSERT INTO todo_order " +
           "(todo_id) " +
           "VALUES (?)";

    result = await exports.query(quer, [result.insertId]);
...
exports.todo.delete = async function(todo_index)
{
    ...
    todo_index = parseInt(todo_index);

    quer = "DELETE todo, todo_order " +
           "FROM todo " +
           "INNER JOIN todo_order ON todo.todo_id = todo_order.todo_id " +
           "WHERE todo_order.todo_index = ?"
    result = await exports.query(quer, [todo_index]);
...
exports.todo.approve = async function(todo_index)
{
    ...
    assert_nonnull(todo_index, "todo_index");
    todo_index = parseInt(todo_index);
    condition = "todo_index = ?";
    values_array.push(todo_index);

    quer = "UPDATE todo " +
           "SET todo.todo_pending = false " +
           "WHERE todo.todo_pending = true " +
           "AND todo.todo_id = (" +
           "SELECT todo_order.todo_id " +
           "FROM todo_order " +
           "WHERE todo_order.todo_index = ?)"
